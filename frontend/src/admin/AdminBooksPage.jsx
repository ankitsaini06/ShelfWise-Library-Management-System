...
import { adminBooksPageStyles as s } from "../dummyStyles";
...
const AdminBooksPage = () => {
  ...
  const canSearchRoll =
    issueForm.rollNumber.trim().length > 0 &&
    !isStudentSelected;
      useEffect(() => {
    ...
    searchTimeoutRef.current = window.setTimeout(async () => {
      try {
        ...
        if (response.ok && data.success) {
          ...
          setSearchError(
            data.message ||
              "Unable to search students by roll number."
          );
        }
      } catch (error) {
        console.error(
          "Student roll search error:",
          error
        );

        setMatchingStudents([]);
        setSearchError(
          "Unable to fetch matching students."
        );
      } finally {
  ...
  const handleIssueSubmit = async (
    ...
    const result =
      await issueManualBooksToStudent({
    ...
    if (!result.ok) {
      setFormMessage(
        result.error ??
          "Unable to issue books right now."
      );
      return;
    }
          ...
          <div className={s.headerFlex}>
            <div>
              ...
              <p className={s.subtitle}>
                Select a student, add manual
                book entries with book code,
                and the active overdue fine
                rule will be used
                automatically after the due
                date.
              </p>
            </div>
            ...
            <div className={s.fineRuleBadge}>
              Fine rule: Rs.
              {fineSettings.amount} per{" "}
              {fineSettings.interval}
            </div>
          </div>
...
<div className={s.matchingContainer}>
  ...
  {selectedStudent && (
    ...
    >
      <span
        className={s.selectedStudentBadge}
      >
        ...
        <div className={s.bookCardHeader}>
          <div className={s.bookIndexWrapper}>
            ...
            <p className={s.bookIndexHelper}>
              Add book name and code. Issue
              date is set automatically to
              today.
            </p>
          </div>
