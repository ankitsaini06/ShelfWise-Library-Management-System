...
import { adminUsersPageStyles as s } from "../dummyStyles";
...
const handleConfirm = async () => {
  ...
  if (confirmAction.type === "clear-fine") {
    const result = await clearFineForRecord(
    ...
    if (result?.ok) {
      ...
      triggerToast(
        "Failed to clear fine. Please try again.",
        "error"
      );
    }
  }
  ...
  if (confirmAction.type === "return-book") {
    const result = await returnIssuedRecord(
      confirmAction.payload
    );
